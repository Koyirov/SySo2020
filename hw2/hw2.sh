...
{
    if [ -z "$arg2" ]; then
        artifacts/dropbearmulti dbclient -y -p 22222 -l root -i ~/.ssh/id_dropbear localhost
    else
        artifacts/dropbearmulti dbclient -y -p 22222 -l root -i ~/.ssh/id_dropbear localhost $arg2
    fi
}
